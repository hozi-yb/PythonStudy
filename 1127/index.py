...
# 평균기온 계산 함수
def avg_temperatures(weather_data): # weather_data는 전역변수이지만 명시적으로 매개변수로 넣어주자. 매개변수로 안적어도 작동은 하지만 깔끔한 코드를 위해.
...
def all_data(weather_data):
    print("\n현재 저장된 날씨 데이터 :")
    for data in weather_data:
        print(f"날짜 : {data[0]}, 도시 : {data[1]}, 평균 기온 : {data[2]}℃ , 강수량 : {data[3]} ")
